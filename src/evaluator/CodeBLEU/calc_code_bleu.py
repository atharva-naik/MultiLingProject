...
def codebleu_fromstr(refs, hyp, lang, params='0.25,0.25,0.25,0.25'):
    alpha, beta, gamma, theta = [float(x) for x in params.split(',')]

    # preprocess inputs
    references = refs #[[x.strip() for x in open(file, 'r', encoding='utf-8').readlines()] for file in refs]
    hypothesis = hyp # [x.strip() for x in open(hyp, 'r', encoding='utf-8').readlines()]
